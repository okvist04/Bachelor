...
Bitcoin_data <- read.table("BitcoinDataCoinDesk.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)

closing_price <- Bitcoin_data[,3]
#date <- as.Date(Bitcoin_data$Date, "%Y-%m-%d")
...
forecast <- forecast::forecast(fit, h = 100)
