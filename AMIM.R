...
AMIM.all <- (mim.all - 0.9184596)/(1 - 0.9184596); AMIM.all 
# AMIM = 0.3481811, hence inefficient market overall
...
mim <- bitcoin %>%
  ...
  #dplyr::group_by(identifier = year) %>% # yearly
  dplyr::group_by(identifier = ym) %>% # monthly
  # dplyr::group_by(identifier = date) %>% # daily #These three can be used as wished
  dplyr::group_split() %>%
  purrr::map_dfr(
    .f = function(dat) {
      # browser()
...
#################################################################
# The code run with monthly instead of yearly 
#################################################################
...
# Combining all the monthly AMIM values. 
AMIM.monthly <- cbind(0, AMIM.nov2013, 0, 0, AMIM.feb2014, 0, 0, 0, 0, AMIM.jul2014, 0, 0, 0, AMIM.nov2014, 
                     ...
                     AMIM.nov2020, 0, 0, 0, AMIM.mar2021, 0) 
# In total 24 months out of 91 are inefficient. 
# Plotting the monthly AMIM, with a horizontal line to illustrate the limit of effeciency
